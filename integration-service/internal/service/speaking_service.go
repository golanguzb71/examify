	...
	session.History = []*genai.Content{
		{
			...
			Parts: []genai.Part{
				...
				genai.Text(fmt.Sprintf("Analyze the audio for the following question as ielts speaking rate 1 to 7 as ielts  : %s.", question)),
			},
		},
	}

	resp, err := session.SendMessage(ctx, genai.Text("Please provide valid JSON format with all required schema fields as IELTS band scores 1 to 7 for speaking."))
	if err != nil {
